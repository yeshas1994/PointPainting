...
def getGraphDataFilename(graphBaseFolderName, idx, extStr):
    filename = graphBaseFolderName + "/navGraph_" + str(idx) + "." + extStr
...
def load_map(path):
    ...
    """
    with open(getGraphDataFilename(path, 0, "spotGraph"), "rb") as graph_file:
        ...
        for waypoint in current_graph.waypoints:
            ...
            file_name = getGraphDataFilename(path, cnt, "spotWaypts")
        ...
        for edge in current_graph.edges:
            file_name = getGraphDataFilename(path, cnt, "spotEdges")
            if not os.path.exists(file_name):
                continue
            with open(file_name, "rb") as snapshot_file:
